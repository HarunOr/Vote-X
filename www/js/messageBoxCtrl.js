   ...
   if ($rootScope.userInfo) {
     ...
     if ($rootScope.currentUserSignedIn) {
         ...
         $scope.getUserInfo = function(id, index) {
           userRef.child(id).once("value", function(user) {
             ...
             $scope.tempArray[index] = {
               ...
               ownProfileImage: userData.ownProfileImg,
               profileImage: "",
               read: ""
             };
             ...
             if ($scope.tempArray[index] !== undefined && $scope.tempArray[index].ownProfileImage) {

               $scope.getImage(id);
               $timeout(function() {
                 $rootScope.requestImg.on("success", function(resp) {
                   $scope.tempArray[index].profileImage = resp.data.Body.toString();

                   $rootScope.messageArray[index] = $scope.tempArray[index];
                   $ionicLoading.hide();
                 });
               });
             } else if ($scope.tempArray[index] !== undefined && !$scope.tempArray[index].ownProfileImage) {
               $timeout(function() {

                 $scope.tempArray[index].profileImage = userData.profileImage;
                 $rootScope.messageArray[index] = $scope.tempArray[index];
                 $ionicLoading.hide();
               });
             } else {
       ...
       $scope.openMessages = function(index) {
         ...
         userRef.child($rootScope.userInfo.uid).update({
           newMail: false
         });
