 angular.module('starter.loginCtrl', ['firebase', 'ngMessages'])
   .controller("loginCtrl", function($scope, $ionicModal, $rootScope, $firebaseObject, $ionicPopup, $state, $firebaseAuth, $ionicLoading, $firebaseArray, $timeout) {
         ...
         myRef.authWithPassword({
           ...
           if (error) {
             ...
             navigator.geolocation.getCurrentPosition(function (position) {
               $rootScope.userGEO = {lat: position.coords.latitude,
                                     lng: position.coords.longitude
                                    };
             },
                                                  function () {
                                                    alert("Aktivieren Sie ihre GPS-Funktion und starten sie die App neu, um die Entfernung zu den gesuchten Orten bestimmen zu können !");
             ...
             var userFBs = new Firebase("https://vote-x.firebaseio.com/users/" + authData.uid);
             //Get userdata to display
             userFBs.once("value", function(snapshot) {
               $scope.$apply(function() {
                 $scope.userData = snapshot.val();
                 $rootScope.user.level = $scope.userData.level;
                 $rootScope.user.uid = authData.uid;
                 $rootScope.user.upvotePoints = $scope.userData.upvotePoints;
     ...
     if (authDatas && !$rootScope.currentUserSignedIn) {
       ...
       userFB.set(hours + ":" + minutes + " " + day + "/" + (month + 1) + "/" + year);
       $rootScope.currentUserSignedIn = true;
       var userFBd = new Firebase("https://vote-x.firebaseio.com/users/" + authDatas.uid);



       //Get userdata to display
       userFBd.once("value", function(snapshot) {
     ...
     $scope.getS3 = function() {
       ...
       loadCred.$loaded().then(function() {
         ...
         AWS.config.update({
           accessKeyId: $rootScope.creds.access_key,
           secretAccessKey: $rootScope.creds.secret_key
         });
